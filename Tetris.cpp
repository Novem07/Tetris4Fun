	...
	public:
		TetrisGame(int width, int height, int info, int screenWidth, int screenHeight);
		~TetrisGame();
	...
	bool TetrisGame::DoesPieceFit(int nTetromino, int nRotation, int nPosX, int nPosY)
	{
		...
		for (int px = 0; px < 4; px++)
			for (int py = 0; py < 4; py++)
			{
				...
				int fi = (nPosY + py) * (nFieldWidth + nFieldInfo) + (nPosX + px);

				// Check that test is in bounds. Note out of bounds does
				// not necessarily mean a fail, as the long vertical piece
				// can have cells that lie outside the boundary, so we'll
				// just ignore them
				if (nPosX + px >= 0 && nPosX + px < (nFieldWidth + nFieldInfo))
				{
	...
	TetrisGame::TetrisGame(int width, int height, int info, int screenWidth, int screenHeight)
		...
		nScore(0),
		bGameOver(false) {
		pField = new unsigned char[(nFieldWidth + nFieldInfo) * nFieldHeight];
		for (int x = 0; x < nFieldWidth + nFieldInfo; x++)
			for (int y = 0; y < nFieldHeight; y++) {
				pField[y * (nFieldWidth + nFieldInfo) + x] = (x == 0 || x == nFieldWidth - 1 || y == nFieldHeight - 1) ? 9 : 0;
			}
	...
	void TetrisGame::RunGame() {
		...
		while (!bGameOver) {
					...
					for (int px = 0; px < 4; px++)
						for (int py = 0; py < 4; py++)
							if (tetromino[nCurrentPiece][Rotate(px, py, nCurrentRotation)] != L'.')
								pField[(nCurrentY + py) * (nFieldWidth + nFieldInfo) + (nCurrentX + px)] = nCurrentPiece + 1;
					...
					for (int py = 0; py < 4; py++)
						if(nCurrentY + py < nFieldHeight - 1)
						{
							...
							for (int px = 1; px < nFieldWidth - 1; px++)
								bLine &= (pField[(nCurrentY + py) * (nFieldWidth + nFieldInfo) + px]) != 0;

							if (bLine)
							{
								// Remove Line, set to =
								for (int px = 1; px < nFieldWidth - 1; px++)
									pField[(nCurrentY + py) * (nFieldWidth + nFieldInfo) + px] = 8;
			...
			for (int x = 0; x < nFieldWidth + nFieldInfo; x++)
				for (int y = 0; y < nFieldHeight; y++)
					screen[(y + 2)*nScreenWidth + (x + 2)] = L" ABCDEFG=#"[pField[y*(nFieldWidth + nFieldInfo) + x]];
				...
				for (auto &v : vLines)
					for (int px = 1; px < nFieldWidth - 1; px++)
					{
						for (int py = v; py > 0; py--)
							pField[py * (nFieldWidth + nFieldInfo) + px] = pField[(py - 1) * (nFieldWidth + nFieldInfo) + px];
						pField[px] = 0;
					}
	...
	int main() {
		...
		TetrisGame tetrisGame(10, 20, 10, 120, 30);
		tetrisGame.RunGame();
